...
from XMusic.utils.database import (get_active_chats,
                                   remove_active_chat,
...
async def update_(client, message, _):
    ...
    for info in repo.iter_commits(
        ...
        updates += f"<b>➣ #{info.count()}: [{info.summary}]({REPO_}/commit/{info}) by -> {info.author}</b>\n\t\t\t\t<b>➥ Dikomit pada:</b> {ordinal(int(datetime.fromtimestamp(info.committed_date).strftime('%d')))} {datetime.fromtimestamp(info.committed_date).strftime('%b')}, {datetime.fromtimestamp(info.committed_date).strftime('%Y')}\n\n"
    _update_response_ = "<b>Pembaruan baru tersedia untuk Bot!</b>\n\n➣ Mulai Pembaruan Sekarang</code>\n\n**<u>Updates:</u>**\n\n"
    _final_updates_ = _update_response_ + updates
    if len(_final_updates_) > 4096:
        url = await Xbin(updates)
    ...
    if await is_heroku():
        ...
        for x in served_chats:
            try:
                ...
                pass
        await response.edit(
            f"{nrs.text}\n\nBot berhasil di update! Sekarang, tunggu 1 - 2 menit hingga bot mulai ulang."
        )
...
async def restart_(_, message):
    ...
    try:
        ...
        pass
    await response.edit(
        "Bot telah berhasil dimulai! Tunggu 1 - 2 menit sampai bot restart."
    )
    os.system(f"kill -9 {os.getpid()} && bash start")
